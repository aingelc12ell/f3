...
final class Base extends Prefab implements ArrayAccess {
			...
			if (isset($route[$this->hive['VERB']]) && !$preflight) {
				...
				if (!strlen($body)) {
					...
					if (isset($cache) && !error_get_last()) {
						...
						$cache->set($hash,[
							// Remove cookies
							preg_grep('/Set-Cookie\:/',headers_list(),PREG_GREP_INVERT),$body,$result
                            ],$ttl);
					}
				}
		...
		foreach ($source as $file) {
			...
			if ($matches) {
				...
				foreach ($matches as $match) {
					if ($match['section']) {
						$sec = $match['section'];
						if (preg_match('/^(?!(?:global|config|route|map|redirect)s\b)(.*?)(?:\s*[:>])/i'
                                ,$sec,$msec)
                            && !$this->exists($msec[1])
                        ) {
	...
	**/
	function mutex($id,$func,$args = NULL) {
		...
		// Use filesystem lock
		if (is_file($lock = $tmp.$this->hive['SEED'].'.'.$this->hash($id).'.lock')
	...
	**/
	...
	function __construct()
    {
		...
		foreach (explode('|',self::GLOBALS) as $global) {
			...
			$this->init += [
				$global => preg_match('/SERVER|ENV/',$global)
                    ? $sync
                    : []
			];
		}
